 ...
 */
DS1307::DS1307(int address, byte statusLedPin) {
  _address = address;
 ...
 */
void DS1307::statusLed(boolean on) {
  if(on) {
...
void DS1307::readTime() {
  byte returnStatus, count, result;
  ...
  DEBUG_PRINT(F("Wire.write returns "));
  DEBUG_PRINTLN(returnStatus);
  DEBUG_FLUSH();
  Wire.requestFrom(_address, 7);
  DEBUG_PRINT(F("Wire.available() = "));
